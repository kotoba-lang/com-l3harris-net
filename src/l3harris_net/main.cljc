(ns l3harris_net.main
  "Kotodama WASM entrypoint for the L3harris_net clean-room actor (L4) — Clojure port.
  ...
  facts (`l3harris_net.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "l3harris_net")
(def tier "L4")
...
(def entity-specs
  [{:entity "Case"      :plural "cases"      :id-prefix "l3harris_cas"
    :fields [:number :jurisdiction :status :filedAt] :required [:number :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Party"     :plural "parties"    :id-prefix "l3harris_par"
    :fields [:caseId :role :name]          :required [:role :name]
    :coerce {}                       :refs {:caseId "Case"}}
   {:entity "Document"  :plural "documents" :id-prefix "l3harris_doc"
    :fields [:caseId :type :contentRef]    :required [:type]
    :coerce {}                       :refs {:caseId "Case"}}
   {:entity "Incident"  :plural "incidents" :id-prefix "l3harris_inc"
    :fields [:type :jurisdiction :occurredAt] :required [:type :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Watchlist" :plural "watchlists" :id-prefix "l3harris_wat"
    :fields [:name :program :entries]      :required [:name :program]
    :coerce {:entries :int}          :refs {}}
   {:entity "Sanction"  :plural "sanctions"  :id-prefix "l3harris_san"
...
(defn emit-facts
  "EAVT facts for one record: {\"l3harris_net.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "l3harris_net-compat" :tier tier :entities entities} 200])
...
(defn start! [] :l3harris_net-compat/ready)
